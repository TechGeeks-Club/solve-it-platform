        ...
        try:
            async for message in self.consumer:
                ...
                try:
                    ...
                    logger.debug(
                        f"Result data: status={result.get('status')}, score={result.get('score')}, passed={result.get('passed_tests')}/{result.get('total_tests')}"
                    )
                    ...
                    submission.correction_id = (
                        f"{submission_id}_{int(timezone.now().timestamp())}"
                    )
                    ...
                    logger.info(
                        f"Checking for coin awards for submission {submission_id} status={submission.status}"
                    )
                    if submission.score == 100 and submission.status == "completed":
                        ...
                        logger.info(
                            f"💰 Awarded {coins_awarded} coins to team {team.name} for 100% completion"
                        )
                    ...
                    if submission.status == "completed":
                        ...
                        first_solver_cache_key = (
                            f"first_solver:task-{submission.task.id}"
                        )
                        ...
                        if not cache.get(first_solver_cache_key):
                            ...
                            already_solved = await asyncio.to_thread(
                                lambda: TaskSolution.objects.filter(
                                    task=submission.task, status="completed"
                                )
                                .exclude(id=submission.id)
                                .exists()
                            )
                            if not already_solved:
                                app_settings = await asyncio.to_thread(
                                    Settings.get_settings
                                )
                                bonus_percent = getattr(
                                    app_settings, "first_solver_bonus_percent", 0
                                )
                                submission.score += bonus_percent
                                logger.info(
                                    f"🏆 First solver bonus: score = {submission.score + bonus_percent} to team {submission.team.name} for task {submission.task.id}"
                                )
                        else:
                            logger.info(
                                f"CACHE:No first solver bonus: submission {submission_id} status={submission.status}"
                            )
