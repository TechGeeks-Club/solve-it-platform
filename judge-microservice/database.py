            ...
            async with self.db_pool.acquire() as conn:
                ...
                try:
                    await self.redis_client.setex(
                        cache_key, settings.REDIS_CACHE_TTL, json.dumps(test_cases)
                    )
                except Exception as e:
                    logger.warning(f"Redis cache write error: {e}")

                logger.info(
                    f"Fetched {len(test_cases)} test cases for task {task_id} from database"
                )
                return test_cases
        ...
        try:
            async with self.db_pool.acquire() as conn:
                ...
                if row:
                    ...
                    try:
                        await self.redis_client.setex(
                            cache_key, settings.REDIS_CACHE_TTL, json.dumps(task_info)
                        )
                    except Exception as e:
                        logger.warning(f"Redis cache write error: {e}")
