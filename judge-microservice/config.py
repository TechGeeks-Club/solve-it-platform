...
class Settings(BaseSettings):
    ...
    KAFKA_SUBMISSION_TOPIC: str = Field(
        ...
        description='Kafka topic for incoming submissions'
    )
    KAFKA_RESULT_TOPIC: str = Field(
        default='code-results',
        description='Kafka topic for submission results'
    )
    ...
    JUDGE0_TIMEOUT: int = Field(
        ...
        ge=1,
        le=300,
        description='Judge0 API timeout in seconds'
    )
    ...
    REDIS_CACHE_TTL: int = Field(
        default=3600,
        ge=60,
        description='Redis cache TTL in seconds (1 hour default)'
    )
